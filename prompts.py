...
1. Call session_brief at the start of EVERY new session - returns snapshot (structured context summary) + recent stream entries. If a snapshot roll is needed, it will be triggered automatically in the background.
...
**session_brief()**: Get session context - returns latest snapshot (structured summary of calendar, todos, people, work threads) plus recent stream entries since snapshot date. MUST call at start of every new session. Automatically triggers a background snapshot roll if the stream has grown too large.
